...
def loadData(inputFile_):
    ...
    Args:
        inputFile_: 入力ファイルパス（INPUTPATH相対）
    
    Returns:
        読み込んだDataFrame（date_time列をdatetime型に変換済み）
    """
    ret_ = pd.read_csv( INPUTPATH  + inputFile_)
    # picked_timestamp_utc列があれば除外（比較には不要）
    if 'picked_timestamp_utc' in ret_.columns:
        ret_ = ret_.drop(columns=['picked_timestamp_utc'])
    ret_ = ret_.dropna()
